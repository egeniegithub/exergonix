...
function setCountyMap(mapName) {
    d3.csv(fips_data_file).get(function(error, rows) { 
      ...
      for ( var i in map_ids ) {
        ...
        states_area.push({'id':area_code, 'color':'#000000', 'groupId':ea_code,});
      }
      ...
      // var area = map.getObjectById("C36061");
      // area.color = '#FF0000';
  ...
  d3.csv(fips_data_file).get(function(error, data) { 
    ...
    var k = 0;
      for (var i = 0; i < data.length; i++) {
        ...
        if(fips.length < 5){
          // var nfips = 0 + fips;
          // console.log('fip is less then 5 digit the current lenth is '+fips.length+' and old fips is '+fips+" new fips is "+nfips);
          fips = 0 + fips;
        }
        ...
        data_state_name = data_state_name.toLowerCase();
        // console.log(map_short_ids);
        // if(-1 > (jQuery.inArray( data_state_name, mapName )))/{
          
        // }

        if ((jQuery.inArray( data_state_name, mapName )) > -1){
          // console.log(data_state_name+ " is excit in this array "+ mapName +" "+jQuery.inArray( data_state_name, mapName )+" and eEA code is "+ea_code);
          ea_areas.push({
            //'value':fips,
            'id': 'C'+fips,
            'groupId':ea_code,
    ...
    for ( var i in map_ids ) {
        var area_code = map_ids[ i ];
        ea_groups_dddd.push({'id':area_code, 'color':'#000000', 'groupId':ea_code});
    }
